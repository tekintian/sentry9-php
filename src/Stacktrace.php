...
class Stacktrace
{
    ...
    public static function getStackInfo($frames,
                                          $trace = false,
                                          $errcontext = null,
                                          ...
                                          ReprSerializer $reprSerializer = null)
    {
        $serializer = $serializer ?: new Serializer();
        $reprSerializer = $reprSerializer ?: new ReprSerializer();
        ...
        for ($i = 0; $i < count($frames); $i++) {
            ...
            $context['filename'] = self::stripPrefixes($context['filename'], $stripPrefixes);
            if ($i === 0 && isset($errcontext)) {
                // If we've been given an error context that can be used as the vars for the first frame.
                $vars = $errcontext;
            } else {
                if ($trace) {
                    $vars = self::getFrameContext($nextframe, $frame_var_limit);
                } else {
                    $vars = array();
                }
            }
    ...
    public static function getDefaultContext($frame, $frame_arg_limit = Client::MESSAGE_LIMIT)
    {
        if (!isset($frame['args'])) {
            return array();
        }
        ...
        foreach ($frame['args'] as $arg) {
            $args['param'.$i] = self::serializeArgument($arg, $frame_arg_limit);
            $i++;
        }
        return $args;
    }

    public static function getFrameContext($frame, $frame_arg_limit = Client::MESSAGE_LIMIT)
    {
        if (!isset($frame['args'])) {
            return array();
        }
        ...
        if (!isset($frame['function'])) {
            return self::getDefaultContext($frame, $frame_arg_limit);
        }
        ...
        if (in_array($frame['function'], self::$statements)) {
            if (empty($frame['args'])) {
                ...
                return array(
                    'param1' => self::serializeArgument($frame['args'][0], $frame_arg_limit),
                );
            }
        }
        try {
        ...
        foreach ($frame['args'] as $i => $arg) {
            $arg = self::serializeArgument($arg, $frame_arg_limit);
            if (isset($params[$i])) {
    ...
    private static function serializeArgument($arg, $frame_arg_limit)
    {
        if (is_array($arg)) {
            $_arg = array();
    ...
    private static function stripPrefixes($filename, $prefixes)
    {
        if ($prefixes === null) {
            return $filename;
        }
    ...
    private static function readSourceFile($filename, $lineno, $context_lines = 5)
    {
        $frame = array(
            'prefix' => array(),
            'line' => '',
