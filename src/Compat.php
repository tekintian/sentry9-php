...
class Compat
{
    ...
    public static function hashHmac($algo, $data, $key, $raw_output = false)
    {
        ...
        return self::_hashHmac($algo, $data, $key, $raw_output);
    }
     ...
     */
    public static function _hashHmac($algo, $data, $key, $raw_output = false)
    {
    ...
    public static function jsonEncode($value, $options = 0, $depth = 512)
    {
        ...
        return self::_jsonEncode($value, $depth);
     ...
     * @return string|false
     */
    public static function _jsonEncode($value, $depth = 513)
    {
        if (ini_get('xdebug.extended_info') !== false) {
            ini_set('xdebug.max_nesting_level', 2048);
        }
        return self::_jsonEncodeLowlevel($value, $depth);
    }
     ...
     * @return string|false
     */
    private static function _jsonEncodeLowlevel($value, $depth)
    {
        ...
        if ($isList) {
            foreach ($value as $v) {
                $this_value = self::_jsonEncode($v, $depth - 1);
                if ($this_value === false) {
                    return false;
                }
                $result[] = $this_value;
            ...
            foreach ($value as $k => $v) {
                ...
                $result[] = self::_jsonEncode($k, $depth - 1).':'.$this_value;
            }
